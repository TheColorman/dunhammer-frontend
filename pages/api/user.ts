...
import { DBUser, ExtendedSession, DBBackground } from '../../lib/types'

interface M_DBUser extends DBUser {
    user_id: string;
}

interface M_DBBackground extends DBBackground {
...
export default async function handler(
    ...
    try {
        const result = ((await query({
            query: "SELECT `usr`.`id` as `user_id`, `usr`.`username`, `usr`.`tag`, `usr`.`levelMentions`, `usr`.`levelDm`, `usr`.`disabled`, `usr`.`xp`, `usr`.`level`, `usr`.`coins`, `usr`.`spentMoney`, `usr`.`commandCount`, `usr`.`pingCount`, `usr`.`inviteCount`, `bg`.`id` as `background_id`, `bg`.`name`, `bg`.`description`, `price`, `thumbnail`, `image` FROM `users` as `usr` JOIN `user_background` as `usr_bg` ON `usr_bg`.`user_id` = `usr`.`id` JOIN `backgrounds` as `bg` ON `bg`.`id` = `usr_bg`.`background_id` WHERE `usr`.`id` = ?",
            values: [session.user.discordId]
        })) as Array<M_DBUser & M_DBBackground>)

        // Extract all users backgrounds
        const backgrounds = result.map(({ background_id, name, description, price, thumbnail, image }) => ({ id: background_id, name, description, price, thumbnail, image }))
